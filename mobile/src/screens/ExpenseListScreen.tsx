...
export default function ExpenseListScreen() {
  ...
  const filtered = expenses.filter(e => {
    ...
    const matchSearch = !q || e.merchant.toLowerCase().includes(q) || e.description.toLowerCase().includes(q);
      ...
      {loading ? <ActivityIndicator color={COLORS.primary} style={{ marginTop: 40 }} /> : (
        <FlatList
          ...
          renderItem={({ item: e }) => {
            ...
            const dt    = new Date(e.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' });
            return (
              <View style={s.item}>
                ...
                <View style={s.info}>
                  ...
                  <Text style={s.cat}>{e.category} · {dt}</Text>
                </View>
                <Text style={s.amt}>{fmt(e.amount)}</Text>
                <TouchableOpacity onPress={() => handleDelete(e.id)} style={{ marginLeft: 10 }}>
...
const s = StyleSheet.create({
  ...
  searchWrap: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', margin: 16, marginBottom: 8, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 10, elevation: 1 },
  ...
  item:   { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', borderRadius: 14, padding: 14, elevation: 1, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 4 },
